...
public class RestfullIntegratedServicesObjRunTime implements IntegratedServicesObjRunTime {

	Logger logger = LogManager.getLogger(RestfullIntegratedServicesObjRunTime.class);
	...
	public Map<String, Object> doRequest(Map<String, Object> map,Task task) throws MapExpressionEvaluationException, InvalidHttpRequestMethod, HttpResponseToMapConvertException, FailedRequestException {
		...
		try {
			...
			e.printStackTrace();
			throw new FailedRequestException(String.format("FailedRequestException - Task Name:%s", task.getName()));
		}
		...
		case "JSON":
			try {
				...
				return JsonToMapConverter.convertNodesFromJsonObj(response);
				...
				e.printStackTrace();
				throw new HttpResponseToMapConvertException(String.format("JSON HttpResponseToMapConvertException - Task Name:%s", task.getName()));
			}
		...
		case "XML":
			try {
				...
				return XmlToMapConverter.convertXmlToFlattenedMap(response);
				...
				e.printStackTrace();
				throw new HttpResponseToMapConvertException(String.format("XML HttpResponseToMapConvertException - Task Name:%s", task.getName()));
			}
		}
